...
Input images: Image 1 is the only edit target, the [TL/TR/BL/BR] overlapping tile. Image 2 is the complete 4K coordinate-base reference; use it only to preserve global composition, geometry, colors, lighting and continuity. Return only the enhanced version of Image 1.

Tile coordinates: Image 1 corresponds exactly to [x0, y0, x1, y1] on Image 2. Its shared overlap edges are [right/left/top/bottom descriptions]. Preserve every boundary feature so it continues naturally into adjacent tiles.
...
若某块第一次生成发生漂移，第二次仍使用原始切片作为 Image 1，只加强发生漂移的单一约束，不增加新场景描述。
